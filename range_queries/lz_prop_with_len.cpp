...
struct segtree
{
	...
	void apply_modify_op(ll &a, ll b, ll len) {
		a = modify_op(a, b, len);
	}
	void propagate(int x, int lx, int rx) {
		if (rx - lx == 1)return;
		int m = (rx + lx) / 2;
		apply_modify_op(val[2 * x + 1], ops[x], m - lx);
		apply_modify_op(ops[2 * x + 1], ops[x], 1);
		apply_modify_op(val[2 * x + 2], ops[x], rx - m);

		apply_modify_op(ops[2 * x + 2], ops[x], 1);
		ops[x] = NO_OPERATION;
	...
	void modify(int l, int r, ll v, int x, int lx, int rx) {
		...
		if (lx >= l && rx <= r) {
			ll len = rx - lx;
			ops[x] = modify_op(ops[x], v, 1); // length one because we will add length when calculate it.
			val[x] = modify_op(val[x], v, len);
			return;
		}
		if (rx - lx == 1)return;
		int mid = (lx + rx) / 2;
